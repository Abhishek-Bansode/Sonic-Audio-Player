...
public class PlayerManager implements MediaPlayer.OnBufferingUpdateListener, MediaPlayer.OnCompletionListener, MediaPlayer.OnPreparedListener {
    ...
    private int currentAudioFocus = MPConstants.AUDIO_NO_FOCUS_NO_DUCK;
                ...
                public void onAudioFocusChange(final int focusChange) {
                    switch (focusChange) {
                        case AudioManager.AUDIOFOCUS_GAIN:
                            currentAudioFocus = MPConstants.AUDIO_FOCUSED;
                            break;
                        case AudioManager.AUDIOFOCUS_LOSS_TRANSIENT_CAN_DUCK:
                            // Audio focus was lost, but it's possible to duck (i.e.: play quietly)
                            currentAudioFocus = MPConstants.AUDIO_NO_FOCUS_CAN_DUCK;
                            break;
                        case AudioManager.AUDIOFOCUS_LOSS_TRANSIENT:
                            // Lost audio focus, but will gain it back (shortly), so note whether
                            // playback should resume
                            currentAudioFocus = MPConstants.AUDIO_NO_FOCUS_NO_DUCK;
    ...
    public void registerActionsReceiver() {
        ...
        intentFilter.addAction(MPConstants.CLOSE_ACTION);
    ...
    public void attachService() {
        if (notificationManager != null) {
            playerService.startForeground(MPConstants.NOTIFICATION_ID, notificationManager.createNotification());
        }
    }
    ...
    public void pauseMediaPlayer() {
        ...
        playerService.stopForeground(false);
        notificationManager.getNotificationManager().notify(MPConstants.NOTIFICATION_ID, notificationManager.createNotification());
    }
    ...
    private void configurePlayerState() {
        ...
        if (currentAudioFocus == MPConstants.AUDIO_NO_FOCUS_NO_DUCK) {
            // We don't have audio focus and can't duck, so we have to pause
            pauseMediaPlayer();
        } else {

            if (currentAudioFocus == MPConstants.AUDIO_NO_FOCUS_CAN_DUCK) {
                // We're permitted to play, but only if we 'duck', ie: play softly
                mediaPlayer.setVolume(MPConstants.VOLUME_DUCK, MPConstants.VOLUME_DUCK);
            } else {
                mediaPlayer.setVolume(MPConstants.VOLUME_NORMAL, MPConstants.VOLUME_NORMAL);
            }
